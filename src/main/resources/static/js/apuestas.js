...
class WSChatChanel {
    ...
    sendToServer(tipo,user,room,message){
        let msg = `{ "tipo": "${tipo}", "sala": "${room}" , "nombreusuario": "${user}" , "message": "${message}" }`;
...
chatForm.addEventListener('submit', (e) => {
    ...
    comunnicationWS.sendToServer("userMessage",nombreusuario,chatRoom,msg);
...
comunnicationWS.onOpen = () => comunnicationWS.sendToServer("ApuestasBot",nombreusuario,chatRoom,"has joined.");

// Funciona bien pero no imrpime ni muestra alerts.
window.addEventListener('beforeunload',() => {
    comunnicationWS.sendToServer("ApuestasBot",nombreusuario,chatRoom,"has left.");
});
